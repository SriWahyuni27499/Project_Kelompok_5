...
class Driver_trx_info_belum extends RestController {
    ...
    public function index_post(){
		...
		// $gada = $this->post('status');
		$statusgada=$this->Driver_model->get_status();
		
		if(count($statusgada) <= 0){
			$this->response([
				"status" => false,
				"msg" => "Tidak ditemukan user"
			], 400);
